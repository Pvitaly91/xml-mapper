...
class FeedReconciliationService
{
    ...
    public function summarize(FeedProfile $feedProfile, ?FeedGeneration $generation = null, array $filters = []): array
    {
        ...
        return [
            ...
            'summary' => [
                ...
                'mapped_total' => $feedItems->whereNotIn('status', [FeedItem::STATUS_PENDING, FeedItem::STATUS_INVALID_MAPPING])->count(),
                'ready_total' => $feedItems->where('status', FeedItem::STATUS_READY)->count(),
                'excluded_total' => $feedItems->where('status', FeedItem::STATUS_EXCLUDED)->count(),
                'invalid_total' => $feedItems->whereIn('status', FeedItem::invalidStatuses())->count(),
                'published_total' => $publishedOffers,
                'deltas' => [
                    'source_variants_vs_published' => $sourceVariantsTotal - $publishedOffers,
                    'ready_vs_published' => $feedItems->where('status', FeedItem::STATUS_READY)->count() - $publishedOffers,
                ],
            ],
            'invalid_breakdown' => [
                'invalid_source' => $feedItems->where('status', FeedItem::STATUS_INVALID_SOURCE)->count(),
                'invalid_mapping' => $feedItems->where('status', FeedItem::STATUS_INVALID_MAPPING)->count(),
                'invalid_conformance' => $feedItems->where('status', FeedItem::STATUS_INVALID_CONFORMANCE)->count(),
                'by_error_code' => $validationBreakdown,
            ],
            'top_blockers' => array_slice($validationBreakdown, 0, 8),
            'breakdown' => $this->breakdownRows($feedItems, $blockerFilter),
        ];
    }
     ...
     */
    private function breakdownRows(Collection $feedItems, string $blockerFilter = ''): array
    {
        return $feedItems
            ...
            ->filter(function (array $row) use ($blockerFilter): bool {
                if ($blockerFilter === '') {
                    return true;
                }

                return collect($row['blockers'])->contains(
                    fn (array $blocker): bool => $blocker['code'] === $blockerFilter
                );
            })
            ->values()
            ->all();
